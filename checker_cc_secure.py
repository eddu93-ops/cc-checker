...
class SecureCCChecker:
    ...
    def generate_cc(self, input_data=None, month=None, year=None, cvv=None):
        """Generar tarjeta completa con todos los datos"""
        if not input_data:
        ...
        # Generar o usar fecha proporcionada
        exp_month = month or str(random.randint(1, 12)).zfill(2)
        exp_year = year or str(random.randint(2024, 2028))
        
        # Generar o usar CVV proporcionado
        card_cvv = cvv or str(random.randint(100, 999))
    ...
    def add_complete_manual_card(self):
        """Agregar tarjeta manual completa con todos los datos"""
    ...
    def generate_with_custom_dates_cvv(self):
        """Generar tarjetas con fechas y CVV personalizados"""
        ...
        try:
            ...
            bin_input = input("BIN (6 dígitos - vacío para aleatorio): ") or None
            
            # Fechas personalizadas
            print(f"\n{Fore.YELLOW}--- FECHAS DE EXPIRACIÓN ---")
            month_option = input("Mes específico? (s/n): ").lower()
            if month_option == 's':
                custom_month = input("Mes (MM): ")
            else:
                custom_month = None
            
            year_option = input("Año específico? (s/n): ").lower()
            if year_option == 's':
                custom_year = input("Año (YYYY): ")
            else:
                custom_year = None
            
            # CVV personalizado
            print(f"\n{Fore.YELLOW}--- CVV ---")
            cvv_option = input("CVV específico? (s/n): ").lower()
            if cvv_option == 's':
                custom_cvv = input("CVV (3 dígitos): ")
                if len(custom_cvv) != 3 or not custom_cvv.isdigit():
                    print(f"{Fore.RED}✗ CVV debe tener 3 dígitos")
                    return
            else:
                custom_cvv = None
    ...
    def run(self):
        ...
        print(f"{Fore.YELLOW}✅ Formato: CC | Mes | Año | CVV")
        print(f"{Fore.YELLOW}✅ Fechas y CVV personalizables")
